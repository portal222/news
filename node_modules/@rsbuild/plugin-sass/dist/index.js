...
function unpatchGlobalLocation() {
    global.location?.[GLOBAL_PATCHED_SYMBOL] && delete global.location;
}
    ...
    return id;
        ...
        setup (api) {
            ...
            api.onAfterCreateCompiler(({ compiler })=>{
                ...
                })(pluginOptions.sassLoaderOptions, !!rewriteUrls || isUseSourceMap), rule = chain.module.rule(findRuleId(chain, CHAIN_ID.RULE.SASS)).test(include).resolve.preferRelative(!0).end(), inlineRule = CHAIN_ID.RULE.CSS_INLINE ? chain.module.rule(findRuleId(chain, CHAIN_ID.RULE.SASS_INLINE)).test(include) : null;
                if (CHAIN_ID.RULE.CSS_RAW) {
                    let cssRawRule = chain.module.rules.get(CHAIN_ID.RULE.CSS_RAW);
                    chain.module.rule(CHAIN_ID.RULE.SASS_RAW).test(include).type('asset/source').resourceQuery(cssRawRule.get('resourceQuery'));
                }
