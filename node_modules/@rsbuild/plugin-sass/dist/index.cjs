    ...
    return id;
        ...
        setup (api) {
            ...
            api.onAfterCreateCompiler(({ compiler })=>{
                ...
                var callback;
                let { config } = environment, { sourceMap } = config.output, isUseSourceMap = 'boolean' == typeof sourceMap ? sourceMap : sourceMap.css, { excludes, options } = getSassLoaderOptions(pluginOptions.sassLoaderOptions, !!rewriteUrls || isUseSourceMap), rule = chain.module.rule(findRuleId(chain, CHAIN_ID.RULE.SASS)).test(include).resolve.preferRelative(!0).end(), inlineRule = CHAIN_ID.RULE.CSS_INLINE ? chain.module.rule(findRuleId(chain, CHAIN_ID.RULE.SASS_INLINE)).test(include) : null;
